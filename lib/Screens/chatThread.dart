...
class SingleChatThread extends StatefulWidget {
  ...
  SingleChatThread(
      {this.showMobileView,this.showFndAddDlg,
        this.room,
        this.whileCall,
        this.nameBar,
        this.messagebody,
...
class _SingleChatThreadState extends State<SingleChatThread> {
            ...
            child: Center(
                child: Stack(
                  children: [
                    ...
                    widget.whileCall?Container(width: 0,height: 0,):  Align(
                              ...
                              child: Container(
                                child: new TextField(
                                  onSubmitted: (val) async {
                                    CommonFunctions().sendMessage(widget.socket,widget.auth, val, "txt", "-",
                                        widget.chatBody);
                                ...
                                child: new InkWell(
                                  onTap: () async {
                                    ...
                                    if (result != null) {
                                      ...
                                      if (res["status"]) {
                                        CommonFunctions().sendMessage(widget.socket,
                                            widget.auth,
                                            res["path"],
                                            file.extension,
                                            file.name,
                            ...
                            widget. whileCall == false?   Center(
                              ...
                              Center(
                                child: new InkWell(
                                  onTap: () async {
                                    //widget.room
                                    CommonFunctions().sendMessage(widget.socket,widget.auth,  Emoji.all()[139].char, "txt", "-",
                                        widget.chatBody);
                                  },
                                  child: Padding(
                                    padding: const EdgeInsets.all(0.0),
                    ...
                    itemBuilder: (BuildContext context, int index) {
                      return InkWell(
                          ...
                          onTap: () {
                            ...
                            controller.text =
                                controller.text +widget.selectedEnojis[index].char;
                          },
                          child: Center(
                              child: new Text(widget.selectedEnojis[index].char,
                                  style: TextStyle(fontSize: 22))));
                    },
                  ),
                )
                              ...
                              children: [
                                ...
                                Wrap(
                                 ...
                                 // crossAxisAlignment: CrossAxisAlignment.center,
                                  children: [
                                    InkWell(onTap: (){
                                      forwardThisMessage( socket,snapshot.data[index]);
                                    },
                                      child: Padding(
                                        padding: const EdgeInsets.all(8.0),
  ...
  void forwardThisMessage(IO.Socket socket,dynamic data) {
    ...
    Future<List> getFndList() async {
      var url = Uri.parse(AppSettings().Api_link+'getMyFnds?id=' +
          widget.auth.currentUser.uid);
      var response = await http.get(url);
      return jsonDecode(response.body);
    }
    ...
    Widget allfnds = Container(width: 300,
